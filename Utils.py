...
class mAP:
    def __init__(self, C, S, R, database):
        """[summary]
    ...
    @staticmethod
    # [N, M], [M, K, d]
    def Quantize_AQ_1(codes, codebook, level):
        k, D = codebook.shape
        # [N, D]
        q = np.zeros([codes.shape[0], D])
        for i in range(level):
    ...
    @staticmethod
    # [N, M], [M, K, d]
    def Quantize_RQ(codes, codebook, level, scale):
        k, D = codebook.shape
        # [N, D]
        q = np.zeros([codes.shape[0], D])
        for i in range(level):
            if i > 0:
                rotated = codebook * scale[i - 1]
            else:
                rotated = codebook
            print('Codebook %d' % i)
            q += rotated[codes[:, i]]
    ...
    def AQD_mAP(self, query):
        ...
        for k in [4, 3, 2, 1]:
            # [N, D]
            db = self.Quantize_RQ(self._database.codes, self.C, k, self.S)
            ...
            del db
            del APx
            results += "\nQuantize level %d, AQD mAP@%d: %f" % (k, self.R, result)
        print(results)
        return results
...
if __name__ == "__main__":
    database = Object()
    query = Object()
    codebook = np.random.rand(4, 256, 300)
    l = np.random.rand(54000, 10) > 0.5
    database.label = l.astype(int)
    l = np.random.rand(5000, 10) > 0.5
    query.label = l.astype(int)
    database.codes = np.random.randint(255, size=(54000, 4))
    query.output = np.random.rand(5000, 300)
    map = mAP(codebook, 54000, database)
    map.AQD_mAP(query)
